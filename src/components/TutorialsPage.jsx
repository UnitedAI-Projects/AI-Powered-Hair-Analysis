...
function Navbar({ onNavigate }) {
  ...
  return (
    <nav style={{position:"sticky",top:0,zIndex:100,background:"rgba(248,241,228,0.92)",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",borderBottom:`1px solid ${T.line}`,padding:"18px 48px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <button onClick={()=>onNavigate("landing")} style={{fontFamily:serif,fontSize:32,fontWeight:500,fontStyle:"italic",color:T.redMid,letterSpacing:"-0.5px",background:"none",border:"none",cursor:"pointer",padding:0}}>pelora</button>
      <ul style={{display:"flex",gap:36,listStyle:"none",margin:0,padding:0}} className="pel-desk">
        {NAV.map(n=>(
          <li key={n.screen}><button onClick={()=>onNavigate(n.screen)} style={{color:n.screen==="tutorials"?T.crimson:T.ink,fontFamily:sans,fontSize:14,fontWeight:n.screen==="tutorials"?600:500,letterSpacing:"0.3px",background:"none",border:"none",cursor:"pointer",transition:"color .2s",borderBottom:n.screen==="tutorials"?`2px solid ${T.crimson}`:"none",paddingBottom:n.screen==="tutorials"?2:0}} onMouseEnter={e=>e.currentTarget.style.color=T.crimson} onMouseLeave={e=>e.currentTarget.style.color=n.screen==="tutorials"?T.crimson:T.ink}>{n.label}</button></li>
        ))}
      </ul>
      <button onClick={()=>{onNavigate("quiz")}} className="pel-desk" style={{background:T.crimson,color:T.ivory,padding:"10px 22px",borderRadius:999,fontFamily:sans,fontSize:13,fontWeight:500,letterSpacing:"0.5px",border:"none",cursor:"pointer",transition:"background .2s, transform .2s"}} onMouseEnter={e=>{e.currentTarget.style.background=T.crimsonDeep;e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={e=>{e.currentTarget.style.background=T.crimson;e.currentTarget.style.transform=""}}>Take the Quiz</button>
      <button className="pel-mob" onClick={()=>setMob(o=>!o)} style={{display:"none",background:"none",border:"none",cursor:"pointer",padding:8,flexDirection:"column",gap:5}}>
        {[0,1,2].map(i=><div key={i} style={{width:22,height:1.5,background:T.ink,borderRadius:2}}/>)}
      </button>
      {mob&&<div style={{position:"fixed",top:69,left:0,right:0,background:"rgba(248,241,228,0.98)",borderBottom:`1px solid ${T.line}`,display:"flex",flexDirection:"column",padding:"12px 0",zIndex:99}}>
        {NAV.map(n=><button key={n.screen} onClick={()=>{onNavigate(n.screen);setMob(false)}} style={{padding:"14px 32px",textAlign:"left",background:"none",border:"none",cursor:"pointer",fontFamily:sans,fontSize:15,color:T.ink}}>{n.label}</button>)}
      </div>}
      <style>{`@media(max-width:900px){.pel-desk{display:none!important}.pel-mob{display:flex!important}}`}</style>
    </nav>
  );
}
...
function TutorialCard({ tutorial, index, locked }) {
  ...
  const diffColor = tutorial.difficulty === "advanced" ? T.crimsonDeep : tutorial.difficulty === "intermediate" ? T.crimsonSoft : T.redMid;

  return (
    <article
      style={{
        flex:"0 0 300px", background:T.ivory, borderRadius:14,
        overflow:"hidden", scrollSnapAlign:"start", cursor:"pointer",
        transition:"transform .28s ease, box-shadow .28s ease",
        display:"flex", flexDirection:"column",
        border:`1px solid ${hovered?"rgba(122,16,36,0.12)":"transparent"}`,
        position:"relative",
        transform: hovered ? "translateY(-6px)" : "none",
        boxShadow: hovered ? "0 20px 40px -18px rgba(122,16,36,0.25)" : "none",
        opacity: locked ? 1 : 1,
      }}
    ...
    >
      {/* Media */}
      <div style={{position:"relative",aspectRatio:"16/10",background:`linear-gradient(135deg,${T.creamWarm},${T.redFaint})`,overflow:"hidden",filter:locked?"saturate(0.6) brightness(0.92)":"none"}}>
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",color:T.crimson,opacity:0.85}}
          dangerouslySetInnerHTML={{__html:illus}}
        />
        {/* Lock overlay */}
        {locked&&<>
        ...
        {/* Bookmark */}
        {!locked&&(
          <button onClick={e=>{e.stopPropagation();setSaved(s=>!s)}} style={{position:"absolute",top:10,right:10,width:32,height:32,borderRadius:"50%",background:saved?T.crimson:"rgba(253,250,242,0.92)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:saved?T.redMid:T.crimson,border:"none",transition:"all .2s",zIndex:2}}>
      ...
      <div style={{padding:"18px 20px 20px",flex:1,display:"flex",flexDirection:"column",filter:locked?"opacity(0.55)":"none"}}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:"auto",paddingTop:12,borderTop:`1px dashed ${T.line}`}}>
          {tutorial.tags.map(tag=>(
            <span key={tag} style={{fontFamily:sans,fontSize:10,fontWeight:600,letterSpacing:"0.8px",color:T.crimson,background:"rgba(122,16,36,0.06)",padding:"3px 8px",borderRadius:4}}>{tag}</span>
          ))}
        </div>
      </div>
    </article>
...
// ── Section ───────────────────────────────────────────────────────────────────
function TutorialSection({ eyebrow, title, titleEm, subtitle, items, rowId, personalized }) {
  const rowRef = useRef();
  const scroll = dir => rowRef.current?.scrollBy({left: dir * 340, behavior:"smooth"});

  return (
    <section style={{
      maxWidth:1400, margin:"0 auto", padding:"48px 48px 16px",
      ...(personalized ? {
        background:"linear-gradient(180deg,transparent,rgba(201,163,90,0.08) 40%,rgba(201,163,90,0.08) 60%,transparent)",
        position:"relative",
      } : {}),
    }}>
      {personalized && <div style={{position:"absolute",left:48,right:48,top:0,height:1,background:`linear-gradient(90deg,transparent,${T.redMid},transparent)`}}/>}
      <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:28,paddingBottom:14,borderBottom:`1px solid ${T.line}`}}>
        <div>
          <div style={{fontFamily:sans,fontSize:11,fontWeight:600,letterSpacing:"2px",textTransform:"uppercase",color:T.redMid,marginBottom:6}}>{eyebrow}</div>
          <h2 style={{fontFamily:serif,fontSize:36,fontWeight:500,color:T.ink,letterSpacing:"-0.5px",lineHeight:1.1}}>
            {title} {titleEm && <em style={{color:personalized?T.redMid:T.crimson,fontStyle:"italic"}}>{titleEm}</em>}
          </h2>
          {subtitle && <p style={{fontFamily:serif,fontSize:16,fontStyle:"italic",color:T.muted,marginTop:6}}>{subtitle}</p>}
        </div>
        <div style={{display:"flex",gap:8,flexShrink:0}}>
          {[[-1,"←"],[1,"→"]].map(([dir,arrow])=>(
            <button key={dir} onClick={()=>scroll(dir)} style={{width:38,height:38,borderRadius:"50%",border:`1px solid ${T.line}`,background:T.ivory,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:T.crimson,transition:"all .2s",fontFamily:sans,fontSize:14}} onMouseEnter={e=>{e.currentTarget.style.background=T.crimson;e.currentTarget.style.color=T.ivory;e.currentTarget.style.borderColor=T.crimson;e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={e=>{e.currentTarget.style.background=T.ivory;e.currentTarget.style.color=T.crimson;e.currentTarget.style.borderColor=T.line;e.currentTarget.style.transform=""}}>
  ...
  return (
    <div style={{fontFamily:sans,background:T.cream,color:T.ink,overflowX:"hidden"}}>
      ...
      <section style={{position:"relative",padding:"80px 48px 64px",textAlign:"center",background:`radial-gradient(ellipse at top,rgba(201,163,90,0.18),transparent 60%),radial-gradient(ellipse at bottom,rgba(122,16,36,0.08),transparent 70%),${T.cream}`,borderBottom:`1px solid ${T.line}`,overflow:"hidden"}}>
        <span style={{display:"inline-block",fontFamily:sans,fontSize:11,fontWeight:600,letterSpacing:"2.5px",textTransform:"uppercase",color:T.redMid,padding:"8px 22px",background:"rgba(201,163,90,0.14)",borderRadius:999,marginBottom:28,animation:"pel-fadeUp .8s ease both"}}>The Pelora Tutorial Library</span>
      ...
      {/* Filter bar */}
      <div style={{position:"sticky",top:69,zIndex:90,background:"rgba(253,250,242,0.96)",backdropFilter:"blur(10px)",WebkitBackdropFilter:"blur(10px)",borderBottom:`1px solid ${T.line}`,padding:"18px 48px"}} className="tut-filter-bar">
        ...
        <div className="tut-featured-grid" style={{position:"relative",background:`linear-gradient(135deg,${T.crimsonDeep} 0%,${T.crimson} 55%,${T.crimsonSoft} 100%)`,borderRadius:20,overflow:"hidden",display:"grid",gridTemplateColumns:"1.1fr 1fr",minHeight:360,boxShadow:"0 24px 60px -20px rgba(122,16,36,0.35)"}}>
          ...
          <div style={{padding:"56px 56px 48px",display:"flex",flexDirection:"column",justifyContent:"center",color:T.ivory,position:"relative",zIndex:1}}>
            <span style={{display:"inline-flex",alignItems:"center",gap:8,fontFamily:sans,fontSize:10,fontWeight:600,letterSpacing:"2.5px",textTransform:"uppercase",color:"rgba(255,255,255,0.85)",marginBottom:20}}>
              <span style={{width:6,height:6,borderRadius:"50%",background:T.redMid,display:"inline-block",boxShadow:`0 0 12px ${T.redMid}`}}/>
      ...
      {/* Tutorial sections */}
      <TutorialSection eyebrow="Your Profile" title="Made for" titleEm="Your Curls" subtitle="Tutorials matched to your 3B profile." items={TUTORIALS.personal} personalized/>
      ...
      <div style={{maxWidth:1400,margin:"40px auto 0",padding:"0 48px"}}>
        <div style={{position:"relative",background:`radial-gradient(ellipse at top right,rgba(201,163,90,0.25),transparent 55%),linear-gradient(135deg,${T.crimsonDeep} 0%,${T.crimson} 100%)`,borderRadius:20,padding:"40px 48px",display:"grid",gridTemplateColumns:"auto 1fr auto",gap:28,alignItems:"center",overflow:"hidden",color:T.ivory}} className="tut-unlock-inner">
          ...
          <div style={{position:"relative",zIndex:1,width:64,height:64,borderRadius:"50%",background:T.redMid,display:"flex",alignItems:"center",justifyContent:"center",color:T.crimsonDeep,boxShadow:"0 0 0 8px rgba(201,163,90,0.2)"}}>
          ...
          <div style={{position:"relative",zIndex:1}}>
            ...
            <h3 style={{fontFamily:serif,fontSize:30,fontWeight:500,letterSpacing:"-0.3px",lineHeight:1.15,marginBottom:4}}>
              All tutorials, <em style={{color:"rgba(255,255,255,0.9)",fontStyle:"italic"}}>zero limits.</em>
            </h3>
      ...
      <section style={{maxWidth:1400,margin:"72px auto 0",padding:"0 48px"}}>
        <div style={{position:"relative",background:T.ivory,border:`1px solid ${T.line}`,borderRadius:20,padding:"48px 56px",display:"grid",gridTemplateColumns:"auto 1fr auto",gap:32,alignItems:"center",overflow:"hidden"}} className="tut-ask-inner">
          <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:`linear-gradient(90deg,${T.crimson},${T.redMid},${T.crimson})`}}/>
          <div style={{width:72,height:72,borderRadius:"50%",background:`linear-gradient(135deg,${T.crimson},${T.crimsonDeep})`,display:"flex",alignItems:"center",justifyContent:"center",color:T.redMid,boxShadow:"0 12px 30px -8px rgba(122,16,36,0.4)"}}>
          ...
          <div>
            <h3 style={{fontFamily:serif,fontSize:28,fontWeight:500,color:T.ink,letterSpacing:"-0.3px",marginBottom:4}}>
              Can't find what you're <em style={{color:T.crimson,fontStyle:"italic"}}>looking for?</em>
            </h3>
      ...
      <footer style={{marginTop:96,padding:"48px 48px 32px",background:T.crimsonDeep,color:"rgba(253,250,242,0.7)",textAlign:"center",fontFamily:serif,fontStyle:"italic",fontSize:15}}>
        <div style={{fontFamily:serif,fontSize:32,fontWeight:500,fontStyle:"italic",color:T.redSoft,letterSpacing:"-0.5px",marginBottom:8}}>pelora</div>
