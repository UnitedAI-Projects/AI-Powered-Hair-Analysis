...
const WEEKS = [
  ...
  { week:4, title:"Protect & Retain", tagline:"Lock in your progress",
    challenges:[
      ...
      {day:30, t:"Day 30 photo — compare to Day 1!",          why:"You've earned this moment. See how far your curls have come.", tip:"Take a photo and compare it to how your hair looked on Day 1."},
    ]},
];
...
function WeekCard({ w, status, done, onOpenModal }) {
  ...
  return (
    <div onClick={status!=="locked" ? ()=>setOpen(o=>!o) : undefined}
      style={{background:cardBg,border:`2px solid ${cardBorder}`,borderRadius:16,padding:"22px 20px",display:"inline-block",textAlign:"left",position:"relative",maxWidth:320,width:"100%",cursor:status!=="locked"?"pointer":"default",transition:"transform 0.2s ease",opacity:status==="locked"?0.6:1}}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
        ...
        {status!=="locked" && (
          <svg style={{width:20,height:20,color:weekColor,transition:"transform 0.3s",transform:open?"rotate(180deg)":"none",flexShrink:0}} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
        )}
      </div>
      <div style={{fontFamily:serif,fontSize:22,fontWeight:700,color:titleColor,lineHeight:1.2,marginBottom:2}}>{w.title}</div>
      {status==="done" && (
        <div style={{display:"inline-flex",alignItems:"center",gap:5,marginTop:8,fontSize:11,fontWeight:500,color:T.redDeep,background:"rgba(122,14,30,0.12)",padding:"4px 12px",borderRadius:100}}>🏅 Badge Earned</div>
      )}
      {open && status!=="locked" && (
        <div style={{marginTop:14,display:"flex",flexDirection:"column",gap:8}}>
          {w.challenges.map((c, ci) => {
            ...
            return (
              <div key={c.day}>
                {isNext && <div style={{background:T.gold,color:T.redDeep,borderRadius:8,padding:"10px 14px",textAlign:"center",fontSize:12,fontWeight:500,marginBottom:4}}>Your next step is ready — let's keep going.</div>}
                <div onClick={e=>{e.stopPropagation();onOpenModal(c.day);}}
                  style={{background:isNext?T.goldPale:(cDone?"transparent":T.creamWarm),border:`1px solid ${isNext?T.gold:T.creamMid}`,borderRadius:10,padding:"12px 14px",fontSize:13,fontWeight:400,color:cDone?T.brownMuted:T.brownText,textDecoration:cDone?"line-through":"none",opacity:cDone?0.6:1,cursor:"pointer",transition:"all 0.2s"}}
      ...
      {WEEKS.map((w, wi) => {
        ...
        return (
          <div key={w.week} style={{position:"relative",display:"flex",alignItems:"flex-start",marginBottom:wi<WEEKS.length-1?60:20}}>
            ...
            <div className="ch-week-wrap" style={isLeft?{marginRight:"auto",paddingRight:60,textAlign:"right",width:"45%"}:{marginLeft:"auto",paddingLeft:60,textAlign:"left",width:"45%"}}>
        ...
        {BADGE_DATA.map((b,i) => {
          ...
          return (
            <div key={b.name} style={{textAlign:"center",width:120}}>
              <div style={{width:90,height:90,borderRadius:"50%",margin:"0 auto 10px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,transition:"transform 0.2s",cursor:"default",background:earned?`linear-gradient(145deg,${T.gold},#8B6914)`:T.creamMid,boxShadow:earned?`0 4px 16px rgba(201,160,60,0.3)`:"none"}}
                onMouseEnter={e=>e.currentTarget.style.transform="scale(1.06)"}
                onMouseLeave={e=>e.currentTarget.style.transform=""}>
                {earned?b.icon:(<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={T.brownLight} strokeWidth="1.5"><rect x="5" y="11" width="14" height="10" rx="2"/><path d="M8 11V7a4 4 0 1 1 8 0v4"/></svg>)}
              </div>
...
function Modal({ day, done, onComplete, onClose }) {
  ...
  return (
    <div onClick={e=>{ if(e.target===e.currentTarget) onClose(); }} style={{display:"flex",position:"fixed",inset:0,zIndex:200,background:"rgba(61,43,31,0.4)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{background:T.cream,borderRadius:20,width:"100%",maxWidth:420,padding:"32px 28px",position:"relative",animation:"ch-fadeUp 0.3s ease"}}>
        ...
        <button onClick={onClose} style={{position:"absolute",top:14,right:14,width:32,height:32,borderRadius:"50%",border:`1.5px solid ${T.redMid}`,background:"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:T.redMid,fontSize:18,fontWeight:300,lineHeight:1,transition:"all 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.background=T.redMid;e.currentTarget.style.color="#fff";}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color=T.redMid;}}>×</button>
        ...
        {isDone?(
          ...
          <button onClick={()=>onComplete(day)} style={{width:"100%",fontFamily:sans,fontSize:14,fontWeight:500,padding:16,border:"none",borderRadius:100,background:`linear-gradient(135deg,${T.redDeep},${T.redMid})`,color:"#fff",cursor:"pointer",transition:"transform 0.2s,box-shadow 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-1px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(122,14,30,0.2)";}} onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";}}>Mark as Complete</button>
        )}
      </div>
    </div>
  );
...
export function ChallengePage({ onNavigate }) {
  ...
  const completeDay = day => { setDone(prev => new Set([...prev, day])); setModalDay(null); };
  return (
    <div style={{fontFamily:sans,background:T.cream,minHeight:"100vh"}}>
      {/* dark variant keeps the red background + gold/white text */}
      <Navbar onNavigate={onNavigate} activeScreen="challenge" variant="dark"/>
      <div style={{background:T.redDeep,padding:"80px 24px 56px",textAlign:"center"}}>
      ...
      {modalDay !== null && <Modal day={modalDay} done={done} onComplete={completeDay} onClose={()=>setModalDay(null)}/>}
    </div>
  );
}