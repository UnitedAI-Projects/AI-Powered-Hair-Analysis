...
// Maps quiz budget answer to the RoutineStep tier filter initial value
// "mix" → null (show all), others → that tier string
const BUDGET_TO_FILTER = {
  drugstore: "drugstore",
  mid:       "mid",
  luxury:    "luxury",
  mix:       null,
...
function TierBadge({ tier }) {
  ...
  return (
    <span style={{fontSize:10,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",color:c.text,background:c.bg,border:`1px solid ${c.border}`,padding:"3px 10px",borderRadius:100}}>
...
function ProductCard({ product, isTop }) {
  ...
  return (
    <div style={{background:"#fff",borderRadius:16,border:`1px solid ${T.creamMid}`,padding:"20px",display:"flex",gap:16,alignItems:"flex-start",position:"relative",boxShadow:isTop?"0 4px 20px rgba(122,14,30,0.07)":"none",transition:"box-shadow 0.2s"}}
      onMouseEnter={e=>e.currentTarget.style.boxShadow="0 6px 24px rgba(122,14,30,0.1)"}
      onMouseLeave={e=>e.currentTarget.style.boxShadow=isTop?"0 4px 20px rgba(122,14,30,0.07)":"none"}>
...
// ── Routine step ──────────────────────────────────────────────────────────────
// defaultTier: the user's budget answer pre-selects the matching tier filter.
// Users can still switch to any other tier or "All tiers" freely.
function RoutineStep({ step, stepNumber, defaultTier }) {
  const [expanded, setExpanded]   = useState(true);
  // Initialize the filter to the user's budget tier (null = all)
  const [activeTier, setActiveTier] = useState(defaultTier ?? null);
  ...
  return (
    <div style={{marginBottom:32}}>
      <button onClick={()=>setExpanded(e=>!e)} style={{width:"100%",display:"flex",alignItems:"center",gap:16,background:"none",border:"none",cursor:"pointer",padding:"16px 0",textAlign:"left"}}>
        <div style={{width:36,height:36,borderRadius:"50%",background:step.unavailable?T.creamMid:`linear-gradient(135deg,${T.redDeep},${T.redMid})`,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:serif,fontSize:16,fontWeight:600,flexShrink:0}}>{stepNumber}</div>
        <div style={{flex:1}}>
        ...
        {step.unavailable ? (
          <div style={{background:T.creamWarm,borderRadius:14,padding:"20px 24px",border:`1px dashed ${T.creamMid}`,display:"flex",alignItems:"center",gap:12}}>
            ...
            <div>
              ...
              <div style={{fontSize:12,color:T.brownLight,fontWeight:400}}>We're curating {TYPE_LABELS[step.product_type]} options for your profile.</div>
            </div>
          </div>
        ) : (
          <>
            {/* Tier filter — always shown when multiple tiers exist.
                The user's budget answer is pre-highlighted but they can switch freely. */}
            {availableTiers.length > 1 && (
              <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
                <span style={{fontSize:11,color:T.brownLight,fontWeight:400,marginRight:4}}>Price tier:</span>
                <button
                  onClick={()=>setActiveTier(null)}
                  style={{fontSize:12,fontWeight:500,padding:"6px 14px",borderRadius:100,border:`1px solid ${activeTier===null?T.redMid:T.creamMid}`,background:activeTier===null?T.redMid:"transparent",color:activeTier===null?"#fff":T.brownMuted,cursor:"pointer",fontFamily:sans,transition:"all 0.2s"}}>
                  All tiers
                </button>
                {availableTiers.map(t => {
                  const isActive   = activeTier === t;
                  const isDefault  = t === defaultTier;
                  return (
                    <button
                      key={t}
                      onClick={()=>setActiveTier(t === activeTier ? null : t)}
                      style={{
                        fontSize:12, fontWeight:500, padding:"6px 14px", borderRadius:100,
                        border:`1px solid ${isActive ? T.redMid : T.creamMid}`,
                        background: isActive ? T.redFaint : "transparent",
                        color: isActive ? T.redDeep : T.brownMuted,
                        cursor:"pointer", fontFamily:sans, transition:"all 0.2s",
                        position:"relative",
                      }}>
                      {TIER_LABELS[t]||t}
                      {/* "Your budget" dot indicator on the default tier */}
                      {isDefault && (
                        <span style={{
                ...
                {defaultTier && (
                  <span style={{fontSize:11,color:T.brownMuted,fontStyle:"italic"}}>
                    ● your budget
                  </span>
                )}
              </div>
    ...
    <div style={{background:T.redFaint,border:`1px solid ${T.redMid}`,borderRadius:14,padding:"16px 20px",marginBottom:28}}>
      <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
        ...
        <div>
          ...
          {warnings.map((w,i)=><div key={i} style={{fontSize:13,fontWeight:400,color:T.brownMuted,lineHeight:1.6,marginTop:i>0?6:0}}>{w}</div>)}
        </div>
      </div>
    </div>
  );
...
export function ProductsPage({ answers, aiResult, onNavigate }) {
  const { routine, warnings, profile } = useMemo(() => {
    try { return runRecommendationEngine(answers, aiResult); }
    catch(e) { console.error("Engine error:", e); return { routine: [], warnings: ["Unable to generate recommendations. Please retake the quiz."], profile: {} }; }
  }, [answers, aiResult]);
  ...
  // The default tier filter for each RoutineStep — derived from the quiz budget answer
  const defaultTier = BUDGET_TO_FILTER[answers.budget] ?? null;
  ...
  return (
    <div style={{fontFamily:sans,background:T.cream,minHeight:"100vh"}}>
      ...
      <Navbar onNavigate={onNavigate} activeScreen="products" quizLabel="Retake Quiz" />
      ...
      <div style={{background:`linear-gradient(135deg,${T.redDeep},${T.redMid})`,padding:"80px 48px 56px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative"}}>
          ...
          <h1 style={{fontFamily:serif,fontSize:"clamp(28px,5vw,48px)",fontWeight:500,lineHeight:1.15,color:"#fff",marginBottom:12,animation:"pel-fadeUp 0.6s ease 0.1s both"}}>
            Products matched to <em style={{fontStyle:"italic",color:T.goldLight}}>your curls.</em>
          </h1>
      ...
      <div style={{maxWidth:780,margin:"0 auto",padding:"48px 24px 80px"}}>
        ...
        <div style={{marginBottom:32}}>
          ...
          <p style={{fontSize:14,fontWeight:400,color:T.brownMuted,lineHeight:1.6}}>
            Showing products in your selected budget. Use the tier filter on any step to compare across price ranges.
          </p>
        </div>
        ...
        <div style={{background:T.creamWarm,borderRadius:20,padding:"32px 28px",textAlign:"center",marginTop:48,border:`1px solid ${T.creamMid}`}}>
          ...
          <p style={{fontSize:14,color:T.brownMuted,lineHeight:1.6,maxWidth:400,margin:"0 auto 20px",fontWeight:400}}>Retake the quiz to update your profile and get fresh recommendations.</p>
